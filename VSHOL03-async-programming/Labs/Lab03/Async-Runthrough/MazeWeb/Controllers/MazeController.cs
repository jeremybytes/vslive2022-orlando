...
public class MazeController : Controller
{
    public async Task<IActionResult> Index(int size, string algo, MazeColor color)
    {
        ...
        Image mazeImage = await GenerateMazeImageAsync(mazeSize, algorithm, color);
        byte[] byteArray = await ConvertToByteArrayAsync(mazeImage);
    ...
    private async Task<Image> GenerateMazeImageAsync(int mazeSize, IMazeAlgorithm algorithm, MazeColor color)
    {
        ...
        Image maze = generator.GetGraphicalMazeAsync(true);
        return maze;
    }

    private static async Task<byte[]> ConvertToByteArrayAsync(Image img)
    {
